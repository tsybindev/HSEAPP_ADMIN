...
export async function editTitleCourse(token: string, course_id: string, title: string) {
    const response = await API.patch(`/courses/${course_id}/`, {
        title: title
    }, {
        headers: {
            'Content-Type': 'application/json',
...
export async function uploadCourseImage(
    ...
    const response = await API.patch(`/courses/${course_id}/image/${image_type}/`, formData, {
        params: {
            course_id: course_id,
...
export async function uploadCourseTemplate(
    ...
    // Отправляем запрос через axios
    const response = await API.patch(`/courses/${course_id}/template/`, formData, {
        params: {
            course_id: course_id,
        },
        headers: {
